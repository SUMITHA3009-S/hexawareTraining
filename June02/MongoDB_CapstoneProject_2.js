...
db.createCollection("inrollments")
...
db.learners.find({exeperience_years:{$gt:2}})
...
db.enrollments.aggregate([{$group:{_id:"$payment.mode",totalRevenue:{$sum:"$payment.amount"}}}])

db.enrollments.aggregate([{$group:{_id:"$course_id",totalRevenue:{$sum:"$payment.amount"}}}])

db.learners.aggregate([{$group:{_id:"$goal",totalLearners:{$sum:1}}}])

db.courses.aggregate([{$group:{_id:"$category",avgPrice:{$avg:"$price" }}}])

db.enrollments.aggregate([{$group:{_id:"$course_id",avgCompletion:{$avg:"$progress.completion_percent" }}}])
...
db.enrollments.aggregate([{$lookup:{from:"learners",localField:"learner_id",foreignField:"learner_idas:"learner"}}])

db.enrollments.aggregate([{$lookup:{from:"courses",localField:"course_id",foreignField:"course_id",as:"course"}}])

db.courses.aggregate([{$lookup:{from:"instructors",localField:"instructor_id",foreignField:"instructor_id",as:"instructor"}}])

db.enrollments.aggregate([{$lookup:{from:"learners",localField:"learner_id",foreignField:"learner_id",as:"learner"}},{
 $lookup:{from:"courses",localField:"course_id",foreignField:"course_id",as:"course"}},{
 $lookup:{from:"instructors",localField:"course_id",foreignField:"instructor_id",as:"instructor"}}
